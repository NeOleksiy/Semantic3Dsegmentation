...
def main(cfg: DictConfig):
    ...
    scheduler = torch.optim.lr_scheduler.MultiStepLR(
        optimizer, milestones=[80, 95], gamma=0.4
    )

    criterion = get_loss(cfg, logger)
    ...
    for epoch in range(epoch, cfg.training.epochs):
        ...
        for i, (feature, mask) in enumerate(progress_bar):
            ...
            progress_bar.set_description(
                f"Train | Epoch {epoch}/{cfg.training.epochs} | Loss {train_loss_meter.avg:.4f} | lr {optimizer.param_groups[0]['lr']}"
            )
            writer.add_scalar(
                "Train/Loss AVG", train_loss_meter.avg, epoch * len(train_loader) + i
            )
            writer.add_scalar("Train/Loss", loss.item(), epoch * len(train_loader) + i)

        test_loss, test_acc, test_miou = validate(
            model, val_loader, cfg.model.num_classes, criterion, device
        )
        logger.info(
            f"Val | Loss {test_loss:.4f} | mIoU: {test_miou:.4f}% | Accuracy: {test_acc:.3f}"
        )
    ...
    plot_segmentation_metrics(
        train_loss=train_loss, val_loss=test_loss_m, val_mean_iou=test_miou_list
    )
