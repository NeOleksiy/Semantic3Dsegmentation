...
class PointNetSetAbstraction(nn.Module):
    ...
    def forward(self, xyz, points):
        """
        Forward pass for set abstraction
        
        Args:
            xyz: (B, 3, N) tensor of point coordinates
            points: (B, D, N) tensor of point features (can be None)
            
        Returns:
            new_xyz: (B, 3, npoint) sampled point coordinates
            new_points: (B, mlp[-1], npoint) extracted features
        """
...
class PointNetFeaturePropagation(nn.Module):
    ...
    def forward(self, xyz1, xyz2, points1, points2):
        """
        Forward pass for feature propagation
        
        Args:
            xyz1: (B, 3, N) target point coordinates (denser)
            xyz2: (B, 3, S) source point coordinates (sparser)
            points1: (B, D, N) target point features (can be None)
            points2: (B, D, S) source point features
            
        Returns:
            new_points: (B, mlp[-1], N) upsampled features
        """
        # Rearrange dimensions for processing
...
class PointNetPlusPlus(nn.Module):
    ...
    def __init__(self, num_classes):
        ...
        self.sa1 = PointNetSetAbstraction(1024, 0.1, 32, 14, [32, 32, 64], False)
        ...
        self.drop1 = nn.Dropout(0.5)         # Regularization
    ...
    def forward(self, xyz):
        ...
        Args:
            xyz: (B, 6, N) input points (xyz + additional features)
            
        Returns:
            (B, num_classes, N) per-point class scores
        """
        # Initial features (xyz + additional features)
        l0_points = xyz          # (B, 6, N)
