    ...
    def forward(self, point_cloud: torch.Tensor) -> torch.Tensor:
        ...
        for batch_idx in range(batch_size):
            unique_voxels, inverse_indices, point_counts = torch.unique(
                voxel_indices[batch_idx], return_inverse=True, return_counts=True, dim=0
            )
        ...
        voxel_features = voxel_features.view(
            batch_size, self.grid_size, self.grid_size, self.grid_size, -1
        ).permute(
            0, 4, 1, 2, 3
        )  # -> (B, C, D, H, W)
...
class VoxelNet(nn.Module):
    ...
    def __init__(
        self, num_classes: int = 20, grid_size: int = 64, dropout: float = 0.5
    ):
        super().__init__()
        self.grid_size = grid_size
    ...
    def _interpolate_features(
        self, voxel_features: torch.Tensor, point_cloud: torch.Tensor
    ) -> torch.Tensor:
        ...
        normalized_points = (
            2 * (normalized_points - coord_min) / (coord_max - coord_min + 1e-6) - 1
        )
