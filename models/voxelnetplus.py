...
class OptimizedVoxelization(nn.Module):
    ...
    def __init__(
        ...
        self.vfe = nn.Sequential(
            nn.Linear(
                4 + extra_feature_channels, 64
            ),  # XYZ (3) + density (1) + RGB (3)
            nn.BatchNorm1d(64),
    ...
    def forward(self, point_cloud: torch.Tensor) -> torch.Tensor:
        ...
        voxel_idx = (
            (norm_coords * (self.grid_size - 1))
            .clamp(0, self.grid_size - 1 - 1e-4)
            .long()
        )
        ...
        for batch_idx in range(batch_size):
            unique_voxels, inverse_indices = torch.unique(
                voxel_idx[batch_idx], dim=0, return_inverse=True, return_counts=False
            )

            valid_mask = (unique_voxels >= 0).all(dim=1) & (
                unique_voxels < self.grid_size
            ).all(dim=1)
            ...
            density = (
                torch.bincount(
                    valid_inverse[valid_points_mask], minlength=len(valid_voxels)
                )
                .float()
                .clamp(max=self.max_points)
                / self.max_points
            )
            ...
            voxel_feat = torch.cat(
                [mean_coords, density.unsqueeze(-1), mean_rgb], dim=-1
            )
            ...
            batch_voxels[
                batch_idx, :, valid_voxels[:, 0], valid_voxels[:, 1], valid_voxels[:, 2]
            ] = voxel_features
...
class EnhancedVoxelNetPlus(nn.Module):
    ...
    def __init__(
        ...
        self.voxelizer = OptimizedVoxelization(
            grid_size, max_points_per_voxel, extra_feature_channels
        )
