...
def colorize_pointcloud(
    points: np.ndarray, labels: np.ndarray, color_map: list
) -> np.ndarray:
    """Colorize point cloud based on class predictions"""
    ...
    valid_labels = np.clip(labels, 0, len(color_map) - 1)

    return color_map[valid_labels]
