...
class PointCloudJitter:
    ...
    def __call__(self, data_dict):
        ...
        if pos.shape[1] > 3:
            coords, colors = pos[:, :3], pos[:, 3:]
            noise = np.clip(
                self.sigma * np.random.randn(*coords.shape), -self.clip, self.clip
            )
            coords += noise
            pos = np.hstack([coords, colors])
        else:
            noise = np.clip(
                self.sigma * np.random.randn(*pos.shape), -self.clip, self.clip
            )
            pos += noise
        data_dict["pos"] = pos
        return data_dict
...
test_transform = transforms.Compose(
    [PointCloudToTensor(), PointCloudNormalize()]  # convert  # tensor
)
